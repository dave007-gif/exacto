...
async function calculateCompositeRate(componentType, quantity) {
    ...
    try {
        ...
        // Plant cost using SMM7_2023 method
        const plantData = await fetchPlantData();
        const equipmentList = formulaConfig.equipment || [];
        const plantCost = SMM7_2023.calculatePlantCost(
            quantity,
            plantData.filter(p => equipmentList.includes(p.equipment))
        );
    ...
    try {
        ...
        for (const component of components) {
            try {
                ...
                // Calculate unit rate with division guard
                const unitRate = component.quantity > 0 
                    ? compositeRate.totalCost / component.quantity
                    : 0;

                // BOQ item creation
                const boqItem = {
                    item: component.itemNumber || Date.now().toString(),
                    description: component.description || component.type,
                    quantity: Number(component.quantity.toFixed(2)),
                    unit: component.unit || "m³",
                    rate: Number(unitRate.toFixed(2)),
                    total: compositeRate.totalCost
                };

                // Update BOQ structure
                boqData.workSections[workSection] = boqData.workSections[workSection] || {
                    description: SMM7_CATEGORIES[workSection]?.description || "Unclassified",
                    items: [],
                    total: 0
                };
        ...
        if (!validateBOQStructure()) {
            alert("Invalid BOQ structure - check console for details");
            return;
        }
